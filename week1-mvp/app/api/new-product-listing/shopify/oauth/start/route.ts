...
import { readShopifyOAuthCredentials } from "@/lib/new-product-listing-shopify-oauth-env";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const redirectUri = new URL("/api/new-product-listing/shopify/oauth/callback", origin).toString();
    const state = crypto.randomBytes(20).toString("hex");
    const session = await getSession();
    session.newProductListingShopifyOAuth = {
      state,
      shopDomain,
      clientId,
      clientSecret,
