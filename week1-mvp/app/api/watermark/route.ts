 ...
 */
const WATERMARK_DIR = path.join(DATA_DIR_PATH, "watermark");
...
/**
 * GET /api/watermark — 返回所有水印列表（所有登录用户可用）
 */
export async function GET() {
  try {
    ...
    const list = entries.map((e) => ({
      ...e,
      previewUrl: `/assets/watermark/${e.id}.png?t=${e.uploadedAt}`,
    }));
...
/**
 * DELETE /api/watermark — 删除水印（仅管理员）
 * body: { id }
 */
