...
export default function ProductListingPage() {
  ...
  const [shopifyConnections, setShopifyConnections] = useState<ShopifyBinding[]>(
    [],
  );
  const [authMode, setAuthMode] =
    useState<ShopifyBinding["authMode"]>(DEFAULT_SHOPIFY_AUTH_MODE);
  const [shopDomain, setShopDomain] = useState("");
  const [accessToken, setAccessToken] = useState("");
  const [clientId, setClientId] = useState("");
  const [clientSecret, setClientSecret] = useState("");
  const [storeDialogOpen, setStoreDialogOpen] = useState(false);
  const [storeSwitchDialogOpen, setStoreSwitchDialogOpen] = useState(false);
  const [newStoreAuthMode, setNewStoreAuthMode] =
    useState<ShopifyBinding["authMode"]>(DEFAULT_SHOPIFY_AUTH_MODE);
  const [newStoreShopDomain, setNewStoreShopDomain] = useState("");
  const [newStoreAccessToken, setNewStoreAccessToken] = useState("");
  ...
  function applyShopifyConnectionState(data: {
    ...
    connection?: ShopifyBinding | null;
    connections?: ShopifyBinding[];
    bound?: boolean;
    shopDomain?: string;
  }) {
    const active =
      data.active ??
      data.connection ??
      (data.bound ? (data as ShopifyBinding) : null);
    ...
    if (active) {
      ...
      setAuthMode(active.authMode || DEFAULT_SHOPIFY_AUTH_MODE);
    } else {
      setShopDomain("");
      setAccessToken("");
      setClientId("");
  ...
  async function reloadShopifyConnections() {
    ...
    return data as { active: ShopifyBinding | null; connections: ShopifyBinding[] };
  }
  ...
  useEffect(() => {
    ...
    async function loadBinding() {
      ...
      try {
        const data = await reloadShopifyConnections();
        if (!alive) return;
        if (data.active) {
  ...
  const newStoreTokenPreview = useMemo(() => {
    const secret =
      newStoreAuthMode === "access_token"
        ? newStoreAccessToken
        : newStoreClientSecret;
    if (!secret.trim()) return "未填写";
    return `••••••••••••${secret.trim().slice(-4)}`;
  ...
  async function testConnection() {
    ...
    try {
      ...
      if (binding && !accessToken.trim() && !clientId.trim() && !clientSecret.trim()) {
        try {
          await reloadShopifyConnections();
        } catch {}
      }
    } catch (e) {
      setConnectionMessage(e instanceof Error ? e.message : String(e));
  ...
  async function saveStoreConnection() {
    if (
      !newStoreShopDomain.trim() ||
      (newStoreAuthMode !== "access_token"
        ? !newStoreClientId.trim() || !newStoreClientSecret.trim()
        : !newStoreAccessToken.trim())
    ) {
      setConnectionMessage(
        newStoreAuthMode !== "access_token"
          ? "请填写店铺域名、客户端 ID 和客户端密钥。"
    ...
    try {
      if (newStoreAuthMode === "oauth_app" || newStoreAuthMode === "client_credentials") {
        const res = await fetch("/api/shopify/oauth/start", {
          ...
          body: JSON.stringify({
            ...
            clientSecret: newStoreClientSecret,
          }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || res.statusText);
      ...
      const res = await fetch("/api/shopify/connections", {
        ...
        body: JSON.stringify({
          ...
          shopDomain: newStoreShopDomain,
          accessToken: newStoreAccessToken,
          clientId: newStoreClientId,
          clientSecret: newStoreClientSecret,
        }),
      });
      ...
      if (!res.ok) throw new Error(data.error || res.statusText);
      applyShopifyConnectionState(data);
      setNewStoreShopDomain("");
      setNewStoreAccessToken("");
      setNewStoreClientId("");
      setNewStoreClientSecret("");
      setStoreDialogOpen(false);
      setSyncState("idle");
      setShopifyProductUrl(null);
      setSyncWarnings([]);
      setConnectionMessage("Shopify 店铺已保存并切换为当前店铺。");
      setLastAction("已切换当前 Shopify 店铺");
    } catch (e) {
      setConnectionMessage(e instanceof Error ? e.message : String(e));
    } finally {
      setSavingBinding(false);
  ...
  async function activateStoreConnection(connectionId: number) {
    if (binding?.id === connectionId) return;
    setSwitchingStoreId(connectionId);
    setConnectionMessage(null);
    try {
      ...
      if (!res.ok) throw new Error(data.error || res.statusText);
      applyShopifyConnectionState(data);
      setStoreSwitchDialogOpen(false);
      setSyncState("idle");
      setShopifyProductUrl(null);
      setSyncWarnings([]);
      const active = data.active as ShopifyBinding | null;
      setConnectionMessage(
        active
          ? `已切换到 Shopify 店铺：${active.shopName || active.shopDomain}`
          : "已切换 Shopify 店铺。",
      );
      setLastAction("已切换当前 Shopify 店铺");
    } catch (e) {
  ...
  async function deleteStoreConnection(connectionId: number) {
    ...
    try {
      ...
      setShopifyProductUrl(null);
      setSyncWarnings([]);
      setConnectionMessage("已删除 Shopify 店铺记录。");
      setLastAction(data.active ? "已更新当前 Shopify 店铺" : "Shopify 已解除绑定");
    } catch (e) {
      setConnectionMessage(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  async function unbindShopify() {
    ...
    try {
      ...
      setClientSecret("");
      setConnectionMessage(
        data.connection ? "已删除当前店铺，并切换到最近的历史店铺。" : "已解除 Shopify 绑定。",
      );
      setConfirmUnbind(false);
      setSyncState("idle");
      setLastAction(data.connection ? "已更新当前 Shopify 店铺" : "Shopify 已解除绑定");
    } catch (e) {
      setConnectionMessage(e instanceof Error ? e.message : String(e));
    } finally {
      setUnbinding(false);
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 md:px-8 py-6 md:py-8">
      ...
      {loadingBinding ? (
        ...
        <div className="space-y-4">
          <BoundStatusCard
            ...
            testing={testing}
            onTestConnection={testConnection}
            onOpenStoreSwitcher={() => setStoreSwitchDialogOpen(true)}
      ...
      <StoreConnectionDialog
        ...
        clientId={newStoreClientId}
        clientSecret={newStoreClientSecret}
        tokenPreview={newStoreTokenPreview}
        saving={savingBinding}
        connectionMessage={connectionMessage}
        onClose={() => setStoreDialogOpen(false)}
        onAuthModeChange={setNewStoreAuthMode}
        onShopDomainChange={setNewStoreShopDomain}
        onAccessTokenChange={setNewStoreAccessToken}
        onClientIdChange={setNewStoreClientId}
        onClientSecretChange={setNewStoreClientSecret}
        onSave={saveStoreConnection}
      />

      <StoreSwitchDialog
        open={storeSwitchDialogOpen}
        connections={shopifyConnections}
        activeId={binding?.id ?? null}
        switchingId={switchingStoreId}
        deletingId={deletingStoreId}
        onClose={() => setStoreSwitchDialogOpen(false)}
        onAddStore={() => {
          setConnectionMessage(null);
          setStoreSwitchDialogOpen(false);
          setNewStoreAuthMode(DEFAULT_SHOPIFY_AUTH_MODE);
          setNewStoreShopDomain("");
          setNewStoreAccessToken("");
          setNewStoreClientId("");
          setNewStoreClientSecret("");
        ...
        onActivate={activateStoreConnection}
        onDelete={deleteStoreConnection}
      />
...
function StoreSwitchDialog({
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      title="切换 Shopify 店铺"
      description="已保存的店铺可直接切换；新增店铺需要输入一次 Shopify 密钥。"
      width="3xl"
      footer={
        <>
...
function StoreConnectionList({
  ...
  if (connections.length === 0) {
    return (
      <div className="rounded-md border border-dashed border-border-subtle bg-bg-tertiary p-4 text-sm text-fg-tertiary">
        暂无历史店铺。添加店铺并保存密钥后，会自动出现在这里。
      </div>
    );
  }

  return (
    <div className="min-h-[236px] max-h-[236px] overflow-y-auto overscroll-contain pr-1">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {connections.map((item) => {
          const active = item.id === activeId;
          const busy = switchingId === item.id || deletingId === item.id;
          return (
            <div
              key={item.id}
              role="button"
              tabIndex={active || busy ? -1 : 0}
              aria-disabled={active || busy}
              onClick={() => {
                if (!active && !busy) onActivate(item.id);
              }}
              onKeyDown={(e) => {
              ...
              className={`h-[114px] overflow-hidden text-left rounded-md border p-2.5 transition-colors ${
                active
                  ? "border-success bg-[var(--success-bg)]"
                  : "border-border-subtle bg-bg-secondary hover:border-brand-300 hover:bg-bg-active"
              } ${active || busy ? "cursor-default" : "cursor-pointer"}`}
            >
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <Store size={14} className="shrink-0 text-fg-tertiary" />
                    <span className="truncate text-[13px] font-semibold leading-5 text-fg-primary">
                      {item.shopName || item.shopDomain}
                    </span>
                  </div>
                  <div className="mt-0.5 truncate text-[11px] leading-4 text-fg-tertiary">
                    {item.shopDomain}
                  </div>
                </div>
                {active ? (
                  <span className="shrink-0 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[11px] leading-4 text-emerald-600">
                    当前
                  </span>
                ) : null}
              </div>
              ...
              <div className="mt-1.5 flex items-center justify-between gap-2">
                <span className="truncate text-[11px] leading-4 text-fg-muted">
                  {active
                    ? "正在用于同步"
                    : switchingId === item.id
                      ? "切换中..."
                      : item.lastTestedAt
                        ? `测试：${formatUnixTime(item.lastTestedAt)}`
                        : "点击切换"}
                </span>
                <span
                  role="button"
                  tabIndex={0}
                  aria-label={`删除 ${item.shopDomain}`}
                  className="inline-flex shrink-0 items-center gap-1 rounded px-1.5 py-0.5 text-[11px] leading-4 text-red-500 hover:bg-red-50"
                  onClick={(e) => {
                    e.stopPropagation();
                    onDelete(item.id);
                  }}
...
function StoreConnectionDialog({
  open,
  authMode,
  shopDomain,
  accessToken,
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      title="添加 Shopify 店铺"
      description="保存后会加密保留在历史店铺里，并自动切换为当前同步目标。"
      width="lg"
      footer={
        <>
          <Button variant="ghost" onClick={onClose}>
    ...
    >
      <div className="space-y-3">
        ...
        <div className="rounded-md border border-border-subtle bg-bg-tertiary p-3 text-xs text-fg-secondary">
          <div className="flex items-center justify-between gap-3">
            <span>{authMode === "access_token" ? "Token 预览" : "密钥预览"}</span>
            <span className="font-mono text-[11px]">{tokenPreview}</span>
          </div>
