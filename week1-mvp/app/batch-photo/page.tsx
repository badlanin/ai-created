...
function BatchPhotoTab({
  ...
  // Step 4 改造（可选纯色姿势 + 1-2 张场景的混合输出模式）：
  // - solidColorEnabled：主背景纯色是否参与本次出图；可单击选中，再单击取消
  // - solidColorHex/Name：纯色背景色值；即使取消选择也保留上一次选择，方便下次恢复
  ...
  useEffect(() => {
    ...
    fetch(`/api/jobs/${prefillJobId}`)
      ...
      .then((data) => {
        ...
        if (params.solid_pose_count === 0 || params.solid_color_enabled === false) {
          setSolidColorEnabled(false);
        } else {
          setSolidColorEnabled(true);
        }
        ...
        if (Array.isArray(extraTextItems)) {
          ...
          for (const it of extraTextItems) {
            const key = `${it.text}\u0000${it.scene_id ?? ""}`;
            map.set(key, {
              text: it.text,
              count: it.variant_total ?? 1,
              scene_id: it.scene_id ?? null,
              scene_name: it.scene_name ?? null,
            });
          }
          const texts = Array.from(map.values());
  ...
  useEffect(() => {
    ...
    const savedSolidEnabled = slotStore.get<boolean>("solidColorEnabled");
    if (typeof savedSolidEnabled === "boolean")
      setSolidColorEnabled(savedSolidEnabled);
        ...
        .map((t) => ({
          ...
          count:
            typeof t.count === "number" && t.count >= 1
              ...
              : 1,
          scene_id:
            typeof t.scene_id === "number" && Number.isFinite(t.scene_id)
              ? t.scene_id
              : null,
          scene_name: t.scene_name || null,
          thumb: t.thumb || null,
        }))
        .slice(0, 2);
  ...
  const customSolidColorActive =
    ...
    !SOLID_COLOR_PRESETS.some(
      (p) => p.hex.toLowerCase() === solidColorHex.toLowerCase(),
    );
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      notifyHelpers.info(
        push,
        `任务已提交`,
        `共 ${totalCount} 张（${pureCount} 纯色 + ${sceneCount + textSceneCount} 场景）· 受 Google quota 限制，预计 ${Math.ceil(totalCount / 2)}+ 分钟`,
      );
    } catch (e) {
      notifyHelpers.error(
        push,
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {SOLID_COLOR_PRESETS.map((c) => {
                      const active =
                        solidColorEnabled &&
                        solidColorHex.toLowerCase() === c.hex.toLowerCase();
                      return (
                        <button
                          key={c.hex}
                          type="button"
                    ...
                    <label
                      ...
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded border text-[12px] cursor-pointer transition-colors ${
                        customSolidColorActive
                          ? "border-brand-400 bg-bg-active text-fg-primary"
                          : "border-border-subtle bg-bg-tertiary text-fg-secondary hover:border-border-strong"
                      }`}
                    >
                      <span>自定义</span>
                      <input
                      ...
                      <span className="text-fg-muted text-[10px]">
                        {solidColorEnabled ? solidColorHex : "未选择"}
                      </span>
                    </label>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  ...
                  {extraTextScenes.length > 0 && (
                    <div className="space-y-2 mb-3">
                      {extraTextScenes.map((entry, idx) => {
                        const referenceScene = entry.scene_id
                          ? scenes.find((s) => s.id === entry.scene_id)
                          : null;
                        const referenceThumb =
                          referenceScene?.image_url || entry.thumb || null;
                        const referenceName =
                          referenceScene?.name || entry.scene_name || null;
                        return (
                          <div
                            key={idx}
                          ...
                          >
                            ...
                            {referenceThumb && (
                              <div className="flex items-center gap-2 mb-2 rounded border border-border-subtle bg-bg-base p-1.5">
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img
                                  src={referenceThumb}
                                  alt={referenceName || "场景参考图"}
                                  className="w-9 h-12 object-cover rounded"
                                />
                                <div className="min-w-0">
                                  <div className="text-[11px] text-fg-secondary truncate">
                                    参考图：{referenceName || "已绑定场景图"}
                                  </div>
                  ...
                  {extraTextScenes.length < 2 && (
                    <>
                      ...
                      {/* 预设网格 — 全部放在一起，不按 group 分类 */}
                      <div className="grid grid-cols-4 gap-1 max-h-[420px] overflow-y-auto pr-1">
                        {textScenePresets.map((p) => {
                          const matchedScene = findSceneForTextPreset(p);
                          return (
                            <button
                              key={p.name}
                              type="button"
                              onClick={() => {
                                ...
                                setExtraTextScenes((prev) => [
                                  ...prev,
                                  {
                                    text: p.text,
                                    count: 1,
                                    scene_id: matchedScene?.id ?? null,
                                    scene_name: matchedScene?.name ?? null,
                                    thumb: p.thumb ?? null,
                                  },
                                ]);
                              }}
