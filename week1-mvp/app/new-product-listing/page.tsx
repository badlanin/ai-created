...
} from "./watermark-position-dialog";
import {
...
} from "@/lib/new-product-listing-draft";
import { fetchWithShopifyDevice } from "@/lib/new-product-listing-shopify-device-client";
import shopifyTaxonomyZhCn from "@/lib/new-product-listing-shopify-taxonomy-zh-cn.generated.json";
...
const DEFAULT_CUSTOM_COLOR_VARIANT_VALUE = "Custom Color";
...
const CATEGORY_METAFIELD_MEMORY_STORAGE_KEY =
  "buqiqi_new_product_listing_category_metafield_memory_v1";
...
const EMPTY_FORM: ProductForm = {
  ...
  variantOptionMetafieldKey: "categoryColor",
  variantGroupByOptionName: "Color",
  publicationIds: [],
};

function getDefaultColorVariantOptions(
  form: Pick<ProductForm, "color" | "categoryColor"> = EMPTY_FORM,
): string[] {
  const generatedColor = splitCategoryMetafieldInputValues(
    form.color || form.categoryColor,
  )[0];
  return [generatedColor, DEFAULT_CUSTOM_COLOR_VARIANT_VALUE].filter(
    (value): value is string => Boolean(value),
  );
}

function buildFixedColorSizeVariantRows(
  form: Pick<
    ProductForm,
    "sku" | "price" | "inventory" | "color" | "categoryColor"
  > = EMPTY_FORM,
): ProductVariantRow[] {
  const colors = getDefaultColorVariantOptions(form);
  return colors.flatMap((color, colorIndex) =>
    DEFAULT_SIZE_VARIANT_OPTIONS.map((size, sizeIndex) => {
      const index = colorIndex * DEFAULT_SIZE_VARIANT_OPTIONS.length + sizeIndex;
      const label = `${color} / ${size}`;
      return {
        id: `variant-default-color-size-${colorIndex}-${sizeIndex}`,
        size: label,
        sku: variantSkuFromProductSku(form.sku, label, index),
        ...
        imageUrl: "",
        imageAlt: label,
        linkedMetafieldValue: color,
        optionValues: [
          {
            optionName: "Color",
            optionMetafieldKey: "categoryColor",
            value: color,
            linkedMetafieldValue: color,
          },
          {
...
function buildFixedColorSizeVariantOptionGroups(
  form: Pick<
    ProductForm,
    "sku" | "price" | "inventory" | "color" | "categoryColor"
  > = EMPTY_FORM,
  rows = buildFixedColorSizeVariantRows(form),
): ProductVariantOptionGroup[] {
  return [
    {
      id: "variant-option-default-color",
      optionName: "Color",
      optionMetafieldKey: "categoryColor",
      values: getDefaultColorVariantOptions(form),
      rows,
    },
    {
...
export default function NewProductListingPage() {
  ...
  const [variantRows, setVariantRows] = useState<ProductVariantRow[]>(
    buildFixedColorSizeVariantRows,
  );
  const [variantOptionGroups, setVariantOptionGroups] = useState<
    ProductVariantOptionGroup[]
  >(() => buildFixedColorSizeVariantOptionGroups(EMPTY_FORM, variantRows));
  ...
  useEffect(() => {
    ...
    async function loadBinding() {
      ...
      try {
        const res = await fetchWithShopifyDevice("/api/new-product-listing/shopify/connection");
  ...
  async function testConnection() {
    ...
    try {
      const res = await fetchWithShopifyDevice("/api/new-product-listing/shopify/test-connection", {
        method: "POST",
  ...
  async function saveBinding() {
    ...
    try {
      if (authMode === "oauth_app" || authMode === "client_credentials") {
        const res = await fetchWithShopifyDevice("/api/new-product-listing/shopify/oauth/start", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            shopDomain,
    ...
    try {
      if (binding.authMode === "access_token" && binding.tokenExpiresAt) {
        const res = await fetchWithShopifyDevice(
          "/api/new-product-listing/shopify/client-credentials-token",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  function resetProductListingContent() {
    ...
    setCleanedAiText("");
    setForm(EMPTY_FORM);
    const fixedVariantRows = buildFixedColorSizeVariantRows();
    setVariantRows(fixedVariantRows);
    setVariantOptionGroups(
      buildFixedColorSizeVariantOptionGroups(EMPTY_FORM, fixedVariantRows),
    );
    setCategoryMetafieldCandidatesForAi({});
    setSyncState("idle");
    setLastAction("已清空新产品上架内容");
    setShopifyProductUrl(null);
    setSyncWarnings([]);
  ...
  async function loadAiShopifyReferenceOptions() {
    ...
    if (
      ...
      requests.push(
        fetchWithShopifyDevice(
          `/api/new-product-listing/shopify/product-organization-options?${new URLSearchParams({
    ...
    requests.push(
      fetchWithShopifyDevice("/api/new-product-listing/shopify/customs-options")
        .then(async (res) => {
  ...
  async function generateAiOutput() {
    ...
    try {
      ...
      const res = await fetch("/api/new-product-listing/ai-output", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt,
  ...
  function applyAiToForm() {
    ...
    const fixedVariantRows = buildFixedColorSizeVariantRows(nextForm);
    setVariantRows(fixedVariantRows);
    setVariantOptionGroups(
      buildFixedColorSizeVariantOptionGroups(nextForm, fixedVariantRows),
    );
    setAiRawText(cleaned);
  ...
  async function syncToShopify(
    ...
    try {
      ...
      const res = await fetchWithShopifyDevice("/api/new-product-listing/shopify/products", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          product: {
  ...
  async function uploadLocalMedia(files: FileList | null) {
    ...
    try {
      ...
      const res = await fetch("/api/new-product-listing/media", {
        method: "POST",
        body: fd,
      });
      const data = (await res.json()) as {
  ...
  async function openWatermarkPicker() {
    ...
    try {
      const res = await fetch("/api/new-product-listing/watermark");
  ...
  async function applyWatermarkPlacement(
    ...
    try {
      const res = await fetch("/api/new-product-listing/watermark/apply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 md:px-8 py-6 md:py-8">
      ...
      <Dialog
        ...
        description="解除后将删除本地保存的店铺域名和 Access Token，新产品上架功能将暂停同步到 Shopify。"
        footer={
          <>
...
function ShopifyCategoryPicker({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setError("");
      fetchWithShopifyDevice(`/api/new-product-listing/shopify/categories?${params.toString()}`, {
...
function ProductFormPanel({
  ...
  useEffect(() => {
    ...
    fetchWithShopifyDevice(
      `/api/new-product-listing/shopify/category-metafields?${params.toString()}`,
      { signal: controller.signal },
    )
      .then(async (res) => {
        const contentType = res.headers.get("content-type") || "";
  ...
  async function loadBackendSellingContexts(panel: ShopifySellingContextPanel) {
    ...
    setBackendSellingContextError("");
    try {
      const res = await fetchWithShopifyDevice("/api/new-product-listing/shopify/selling-contexts");
...
function SyncPanel({
  ...
  async function loadProductOrganizationOptions(force = false) {
    ...
    try {
      ...
      const res = await fetchWithShopifyDevice(
        `/api/new-product-listing/shopify/product-organization-options?${params.toString()}`,
        { signal: controller.signal },
      );
      const contentType = res.headers.get("content-type") || "";
      if (!contentType.includes("application/json")) {
