...
【一致性约束（严格遵守）】
1. 背景一致性：所有 {{n}} 张图的背景必须严格匹配参考图 4。即使姿势描述里出现"走动""倚靠""仪式感"等词，也不要联想或自行生成参考图 4 之外的场景元素
2. 人物一致性：每张图只有 1 个人，就是参考图 3 里的那位模特，所有图共享同样的脸/肤色/发型/眼睛/体型
3. 服装一致性：所有图的服装与参考图 1-2 完全一致
4. 唯一变量：只有姿势和构图可以不同
