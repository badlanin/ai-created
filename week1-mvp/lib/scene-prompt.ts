...
export function buildSceneShootTextWithImage(
  sceneText: string,
  ...
  const sceneClean = sceneText.trim();
  const textConstraint = `SCENE TEXT DESCRIPTION - follow this together with IMAGE 2.
Use IMAGE 2 as the visual reference for architecture, layout, lighting, palette,
materials, props, and spatial mood. Use the text below as the semantic brief.
If the text and IMAGE 2 conflict, keep IMAGE 2's visible spatial structure and
use the text for named colors, materials, styling, and details that are hard to
read from the thumbnail.

${sceneClean}`;
  const mergedHint = [textConstraint, userHint?.trim()]
    .filter(Boolean)
    .join("\n\nUSER EXTRA HINT:\n");

  return buildSceneShootImage(scenePlateName, mergedHint, opts);
}
