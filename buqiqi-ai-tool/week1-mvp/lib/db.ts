...
function migrate(db: Database.Database) {
  ...
  // 文字场景预设 v2：按 scene_new_01..28 重新绑定名称和文案，修复图文错位
  migrateRebindTextScenesV2(db);
...
function migrateShopifyConnectionsDeviceScope(db: Database.Database) {
  const cols = db
    .prepare(`PRAGMA table_info(shopify_connections)`)
    .all() as Array<{ name: string; pk: number }>;
  const userIdColumn = cols.find((c) => c.name === "user_id");
  const hasDeviceId = cols.some((c) => c.name === "device_id");
  const userIdIsPrimaryKey = Boolean(userIdColumn?.pk);

  if (hasDeviceId && !userIdIsPrimaryKey) return;

  const deviceExpr = hasDeviceId
    ? "COALESCE(NULLIF(device_id, ''), 'legacy')"
    : "'legacy'";

  db.exec(`
    DROP TABLE IF EXISTS shopify_connections_v2;
    CREATE TABLE shopify_connections_v2 (
      id               INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id          INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      device_id        TEXT NOT NULL,
      shop_domain      TEXT NOT NULL,
...
/**
 * 文字场景预设 v2（2026-05-30）
 *
 * v1 的 28 条文案和 scene_new_01..28 缩略图不是严格一一对应，
 * 导致批量摄影里用户点白色门厅图，文字却写着粉色门厅。
 * v2 以 thumb_path 为锚点，把每张图对应的名称 / 分组 / 文案同步为
 * lib/text-scene-presets.ts 当前版本。
 */
function migrateRebindTextScenesV2(db: Database.Database) {
  const FLAG = "migrated_text_scenes_rebound_v2";
  const flag = db
    .prepare(`SELECT value FROM settings WHERE key = ?`)
    .get(FLAG) as { value: string } | undefined;
  if (flag?.value === "done") return;
  ...
  try {
    ...
    console.warn(
      "[db] migrateRebindTextScenesV2 加载 text-scene-presets 失败:",
      err,
    );
    return;
  }
  ...
  const rows = presets.map((p, i) => ({
    ...
    thumb_path: p.thumb?.replace(/^\/assets\//, "") ?? null,
    sort_order: (i + 1) * 10,
    tmp_name: `__text_scene_rebind_v2_${String(i + 1).padStart(2, "0")}__`,
  }));

  const renameToTemp = db.prepare(
    `UPDATE text_scenes SET name = @tmp_name WHERE thumb_path = @thumb_path`,
  );
  ...
  const tx = db.transaction(() => {
    for (const row of rows) {
      ...
      renameToTemp.run(row);
    }
    for (const row of rows) {
      if (!row.thumb_path) continue;
      const res = update.run(row);
      if (res.changes > 0) {
        updated += res.changes;
      } else {
        const insertedRes = insert.run(row);
        inserted += insertedRes.changes;
      }
    }
    db.prepare(
      `INSERT OR REPLACE INTO settings (key, value, notes) VALUES (?, 'done', ?)`,
      ...
      FLAG,
      `文字场景预设 v2 图文重新绑定（更新 ${updated} 条，新插 ${inserted} 条）`,
    );
  });
  tx();
  console.log(
    `[db] migrateRebindTextScenesV2: 更新 ${updated} 条，新插 ${inserted} 条（已标记 ${FLAG}=done）`,
  );
}
